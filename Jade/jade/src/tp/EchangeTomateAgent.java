 ...
 */
public class EchangeTomateAgent extends Agent {

	private Logger myLogger = Logger.getMyLogger(getClass().getName());

	private class WaitPingAndReplyBehaviour extends CyclicBehaviour {
		private int nbBanane = 0, nbTomate = 10;
		...
		public void action() {
			...
			if(msg != null){
				...
				if(msg.getPerformative()== ACLMessage.REQUEST){
					...
					if ((content != null) && (content.indexOf("banane") != -1)){
						nbBanane ++;
						myLogger.log(Logger.INFO, "Agent "+getLocalName()+" - Recoit une banane de "+msg.getSender().getLocalName() + ", il a donc " + nbBanane + " bananes.");
						if(nbTomate > 0){
							nbTomate --;
							myLogger.log(Logger.INFO, "Agent "+getLocalName()+" - envoit une tomate à "+msg.getSender().getLocalName() + ", il a donc " + nbTomate + " tomates.");
							reply.setContent("tomate");
						} else {
							myLogger.log(Logger.INFO, "Agent "+getLocalName()+" - n'a plus de tomate, il remercie donc "+msg.getSender().getLocalName() + " pour sa générosité.");
